...
import { Search, MapPin, Clock, Building2, Users, ExternalLink, Briefcase, Filter, X, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
const VagasPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const vagasResponse = await axios.get(`${API_URL}/api/recruitment/jobs`);
                ...
                const cidadesUnicas = [...new Set(vagasResponse.data.map(vaga => vaga.cidade).filter(Boolean))];
                const modalidadesUnicas = [...new Set(vagasResponse.data.map(vaga => vaga.modalidade).filter(Boolean))];
    ...
    const vagasFiltradas = vagas.filter(vaga => {
        ...
        const matchEmpresa = !selectedEmpresa || vaga.empresa_id?.toString() === selectedEmpresa;
        const matchCidade = !selectedCidade || vaga.cidade === selectedCidade;
        const matchModalidade = !selectedModalidade || vaga.modalidade === selectedModalidade;
    ...
    const handlePageChange = (novaPagina) => {
        ...
        document.getElementById('vagas-section')?.scrollIntoView({ behavior: 'smooth' });
    };
                ...
                <main className="pt-16">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                        <div className="text-center">
                            ...
                            <h3 className="text-xl font-semibold text-white mb-2">Carregando vagas</h3>
                            <p className="text-gray-400">Buscando todas as oportunidades disponíveis...</p>
                        </div>
                    </div>
                </main>
            </div>
    ...
    return (
        <div className="min-h-screen bg-black">
            ...
            <main className="pt-16">
                            ...
                            <div className="text-center space-y-6 lg:space-y-8 mb-12">
                                <div className="space-y-4">
                                    <div className="inline-flex items-center gap-3 bg-gradient-to-r from-orange-500/20 to-red-500/20 backdrop-blur-sm rounded-full px-4 py-2 border border-orange-500/20">
                                        ...
                                        <span className="text-orange-200 font-medium text-sm">Oportunidades de Carreira</span>
                                    </div>
                                    ...
                                    <p className="text-lg sm:text-xl text-gray-300 leading-relaxed max-w-3xl mx-auto">
                                        Explore todas as oportunidades disponíveis e encontre a vaga ideal para sua carreira no agronegócio
                                    </p>
                                </div>
                                ...
                                <div className="bg-orange-600/20 text-orange-400 px-6 py-3 rounded-full border border-orange-600/30 inline-flex items-center gap-2">
                                    ...
                                    <span className="font-semibold">{vagasFiltradas.length} vaga{vagasFiltradas.length !== 1 ? 's' : ''} encontrada{vagasFiltradas.length !== 1 ? 's' : ''}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {/* Lista de Vagas */}
                <section id="vagas-section" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
                    {/* Informações da página atual */}
                    {vagasFiltradas.length > 0 && (
                        <div className="mb-6 text-center">
                            <p className="text-gray-400 text-sm">
                                Mostrando {indiceInicial + 1} - {Math.min(indiceFinal, vagasFiltradas.length)} de {vagasFiltradas.length} vagas
                                {totalPaginas > 1 && (
                                    <span className="ml-2">• Página {currentPage} de {totalPaginas}</span>
                                )}
                            </p>
                        </div>
                    )}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <header className="mb-6">
                                                <div className="flex items-start justify-between gap-4 mb-4">
                                                    ...
                                                    <button
                                                        onClick={() => navigate(`/empresa/${vaga.empresa_id}`)}
