...
const WorkEnvironmentSection = ({ workEnvironment, aiReport, generatingReport }) => {
  const hasContent = workEnvironment?.ideal?.length > 0 || workEnvironment?.avoid?.length > 0;
    ...
    <div className="bg-white shadow-sm border border-gray-200 p-6">
      ...
      {generatingReport ? (
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {workEnvironment.ideal && workEnvironment.ideal.length > 0 && (
            <div>
          ...
          {workEnvironment.avoid && workEnvironment.avoid.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-gray-900 mb-3">⚠️ Ambientes a Evitar:</h4>
              <div className="space-y-2">
        ...
        <div className="text-center py-8">
          <p className="text-sm text-gray-500">Gere a análise com IA para ver as recomendações de ambiente de trabalho.</p>
        </div>
      )}
    </div>
  );
